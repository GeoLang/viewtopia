 ...
 * agora (live maps) and ptolemy (projects, datasets, branches), all of
...
// two live maps named Coastline, so the list has to tell them apart by id
const DOCUMENTS = [
...
test('live.list names the live maps, then live.join opens a socket for the one named', async ({
  page,
}) => {
  ...
  await runAction(page, 'live.join', { map: 'Campus twin' });
