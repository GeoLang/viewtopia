<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE qgis PUBLIC 'http://mrcc.com/qgis.dtd' 'SYSTEM'>
<qgis version="3.28.0-Firenze" styleCategories="AllStyleCategories" hasScaleBasedVisibilityFlag="0" minScale="1e+08" maxScale="0">
  <renderer-v2 forceraster="0" type="categorizedSymbol" attr="type" symbollevels="0" enableorderby="0">
    <categories>
      <category value="forest" symbol="0" label="forest" render="true"/>
      <category value="water" symbol="1" label="water" render="true"/>
    </categories>
    <symbols>
      <symbol type="fill" alpha="1" force_rhr="0" name="0" clip_to_extent="1">
        <layer locked="0" class="SimpleFill" enabled="1" pass="0">
          <Option type="Map">
            <Option name="color" value="27,120,55,255" type="QString"/>
            <Option name="outline_color" value="35,35,35,255" type="QString"/>
          </Option>
        </layer>
      </symbol>
      <symbol type="fill" alpha="1" force_rhr="0" name="1" clip_to_extent="1">
        <layer locked="0" class="SimpleFill" enabled="1" pass="0">
          <Option type="Map">
            <Option name="color" value="33,102,172,255" type="QString"/>
            <Option name="outline_color" value="35,35,35,255" type="QString"/>
          </Option>
        </layer>
      </symbol>
    </symbols>
  </renderer-v2>
</qgis>
