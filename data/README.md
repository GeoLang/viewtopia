...
The platform compose file mounts this directory into geokode-index and itinera
at `/data`. Everything here except this README and `addresses.csv` is generated
and gitignored.

| File | What it is |
|------|------------|
| `region.osm.pbf` | The OSM extract. geokode-index builds the geokode index from it, itinera builds its routing graph from it. `scripts/platform-up.sh` downloads it, Monaco by default |
...
| `addresses.csv` | Sample addresses in OpenAddresses form, `LON,LAT,NUMBER,STREET,CITY,REGION,POSTCODE`. The re and verticals compose files pass it to geokode-index as `--addresses` |
