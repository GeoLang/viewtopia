...
export function parseLinks(body: unknown, base: string): StacLink[] {
  ...
  return raw.flatMap((entry) => {
    ...
    if (!href || !rel) return [];
    return [{ rel, href: resolveHref(href, base), title: text(link.title) || undefined }];
  });
}
...
export async function fetchStac(url: string): Promise<unknown> {
  const local = sameOrigin(url);
  const init: RequestInit = local
    ? { headers: apiHeaders() }
...
export function parseItems(body: unknown, base: string): StacItemPage {
  ...
  return { items, nextUrl: linkHref(parseLinks(body, base), 'next') };
}
